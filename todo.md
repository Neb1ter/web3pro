...
## 移动端专项优化（2026-02-24）
- [ ] /crypto-saving 左上角增加返回主页按钮，与 /web3-guide 结构保持一致
- [ ] 两个页面浮动菜单增加可拖拽功能，初始位置与回顶按钮同一水平线
- [ ] 移动端滚动时当前章节标题动态显示在顶部导航栏返回按钮右侧（/crypto-saving）
- [ ] 移动端滚动时当前章节标题动态显示在顶部导航栏返回按钮右侧（/web3-guide）
