      ...
      <nav className="sticky top-0 z-50 border-b backdrop-blur-sm" style={{ background: 'rgba(10,25,47,0.95)', borderColor: 'rgba(255,215,0,0.12)' }}>
        <div className="container mx-auto px-4 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            {/* 返回主页按钮：移动端 + 桌面端均显示 */}
            <Link href="/portal" className="tap-target inline-flex items-center gap-1.5 text-slate-400 hover:text-amber-400 transition-colors text-sm shrink-0">
          ...
          <div className="hidden lg:flex items-center gap-5">
            {CHAPTERS.slice(0, 5).map(c => (
              <button
                ...
                className={`tap-target text-xs font-semibold transition ${activeChapter === c.id ? 'text-amber-400' : 'text-slate-500 hover:text-amber-400'}`}
              >
                {zh ? c.zh : c.en}
              </button>
            ))}
