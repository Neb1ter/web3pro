import { useState } from "react";
import { trpc } from "@/lib/trpc";

/**
 * PublishLog 类型与数据库 publish_logs 表字段严格对齐：
 *  - contentType: "article" | "news"
 *  - contentId:   关联内容 ID
 *  - contentTitle: 内容标题快照（入库时写入，无需 JOIN）
 *  - response:    平台返回信息或错误信息
 */
type PublishLog = {
  id: number;
  contentType: string;
  contentId: number;
  contentTitle: string;
  platform: string;
  status: string;
  response: string | null;
  retryCount: number;
  createdAt: Date | string | null;
  updatedAt: Date | string | null;
};

const STATUS_COLORS: Record<string, string> = {
  success: "bg-green-900/60 text-green-300",
  failed:  "bg-red-900/60 text-red-300",
  pending: "bg-yellow-900/60 text-yellow-300",
  skipped: "bg-slate-700 text-slate-400",
};

const STATUS_ICONS: Record<string, string> = {
  success: "✅",
  failed:  "❌",
  pending: "⏳",
  skipped: "⏭️",
};

const PLATFORM_ICONS: Record<string, string> = {
  telegram: "✈️",
  wechat:   "💬",
  weibo:    "🌐",
  twitter:  "🐦",
  douyin:   "🎵",
  rss:      "📡",
};

const CONTENT_TYPE_LABELS: Record<string, { zh: string; en: string }> = {
  article: { zh: "文章", en: "Article" },
  news:    { zh: "快讯", en: "News" },
};

export function PublishLogsTab({ zh }: { zh: boolean }) {
  const [filterPlatform, setFilterPlatform] = useState("all");
  const [filterStatus, setFilterStatus] = useState("all");

  const listQuery = trpc.publishLogs.list.useQuery({
    platform: filterPlatform === "all" ? undefined : filterPlatform,
    status:   filterStatus  === "all" ? undefined : filterStatus,
    limit: 100,
  });

  const logs = (listQuery.data ?? []) as PublishLog[];

  const platforms = ["all", "telegram", "wechat", "weibo", "twitter", "douyin", "rss"];
  const statuses  = ["all", "success", "failed", "pending", "skipped"];

  const successCount = logs.filter(l => l.status === "success").length;
  const failedCount  = logs.filter(l => l.status === "failed").length;
  const pendingCount = logs.filter(l => l.status === "pending").length;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-white">
          {zh ? "📋 推送日志" : "📋 Publish Logs"}
        </h2>
        <button onClick={() => listQuery.refetch()} className="admin-btn-ghost text-sm">
          {zh ? "🔄 刷新" : "🔄 Refresh"}
        </button>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-white">{logs.length}</div>
          <div className="text-xs text-slate-400 mt-1">{zh ? "总推送次数" : "Total"}</div>
        </div>
        <div className="bg-green-900/20 border border-green-700/30 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-green-300">{successCount}</div>
          <div className="text-xs text-slate-400 mt-1">{zh ? "成功" : "Success"}</div>
        </div>
        <div className="bg-red-900/20 border border-red-700/30 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-red-300">{failedCount}</div>
          <div className="text-xs text-slate-400 mt-1">{zh ? "失败" : "Failed"}</div>
        </div>
        <div className="bg-yellow-900/20 border border-yellow-700/30 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-yellow-300">{pendingCount}</div>
          <div className="text-xs text-slate-400 mt-1">{zh ? "待推送" : "Pending"}</div>
        </div>
      </div>

      {/* Filters */}
      <div className="flex gap-4 flex-wrap">
        <div className="flex gap-1 items-center flex-wrap">
          <span className="text-xs text-slate-500 mr-1">{zh ? "平台:" : "Platform:"}</span>
          {platforms.map(p => (
            <button
              key={p}
              onClick={() => setFilterPlatform(p)}
              className={`text-xs px-2 py-1 rounded transition-all ${
                filterPlatform === p
                  ? "bg-cyan-600 text-white"
                  : "bg-slate-700/60 text-slate-400 hover:text-white"
              }`}
            >
              {p === "all"
                ? (zh ? "全部" : "All")
                : `${PLATFORM_ICONS[p] ?? "📱"} ${p}`}
            </button>
          ))}
        </div>
        <div className="flex gap-1 items-center flex-wrap">
          <span className="text-xs text-slate-500 mr-1">{zh ? "状态:" : "Status:"}</span>
          {statuses.map(s => (
            <button
              key={s}
              onClick={() => setFilterStatus(s)}
              className={`text-xs px-2 py-1 rounded transition-all ${
                filterStatus === s
                  ? "bg-cyan-600 text-white"
                  : "bg-slate-700/60 text-slate-400 hover:text-white"
              }`}
            >
              {s === "all"
                ? (zh ? "全部" : "All")
                : `${STATUS_ICONS[s] ?? ""} ${s}`}
            </button>
          ))}
        </div>
      </div>

      {/* Logs table */}
      {listQuery.isLoading ? (
        <div className="flex justify-center py-12">
          <div className="w-8 h-8 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" />
        </div>
      ) : logs.length === 0 ? (
        <div className="text-center py-12 text-slate-500">
          <div className="text-4xl mb-3">📭</div>
          <p>{zh ? "暂无推送记录" : "No push logs yet"}</p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700 text-slate-400 text-left">
                <th className="py-3 px-3 whitespace-nowrap">{zh ? "时间" : "Time"}</th>
                <th className="py-3 px-3 whitespace-nowrap">{zh ? "类型" : "Type"}</th>
                <th className="py-3 px-3">{zh ? "内容标题" : "Content Title"}</th>
                <th className="py-3 px-3 whitespace-nowrap">{zh ? "平台" : "Platform"}</th>
                <th className="py-3 px-3 whitespace-nowrap">{zh ? "状态" : "Status"}</th>
                <th className="py-3 px-3 whitespace-nowrap">{zh ? "重试" : "Retry"}</th>
                <th className="py-3 px-3">{zh ? "响应信息" : "Response"}</th>
              </tr>
            </thead>
            <tbody>
              {logs.map(log => (
                <tr
                  key={log.id}
                  className="border-b border-slate-800 hover:bg-slate-800/30 transition-colors"
                >
                  {/* 时间 */}
                  <td className="py-2 px-3 text-slate-400 text-xs whitespace-nowrap">
                    {log.createdAt
                      ? new Date(log.createdAt).toLocaleString("zh-CN")
                      : "-"}
                  </td>

                  {/* 内容类型 */}
                  <td className="py-2 px-3">
                    <span className={`text-xs px-2 py-0.5 rounded ${
                      log.contentType === "article"
                        ? "bg-cyan-900/40 text-cyan-300"
                        : "bg-purple-900/40 text-purple-300"
                    }`}>
                      {zh
                        ? (CONTENT_TYPE_LABELS[log.contentType]?.zh ?? log.contentType)
                        : (CONTENT_TYPE_LABELS[log.contentType]?.en ?? log.contentType)}
                    </span>
                  </td>

                  {/* 内容标题（快照，直接显示，无需 JOIN） */}
                  <td className="py-2 px-3 max-w-xs">
                    {log.contentTitle ? (
                      log.contentType === "article" ? (
                        <a
                          href={`/article/${log.contentId}`}
                          target="_blank"
                          className="text-cyan-400 hover:text-cyan-300 text-xs truncate block"
                          title={log.contentTitle}
                        >
                          {log.contentTitle}
                        </a>
                      ) : (
                        <span className="text-slate-300 text-xs truncate block" title={log.contentTitle}>
                          {log.contentTitle}
                        </span>
                      )
                    ) : (
                      <span className="text-slate-500 text-xs">ID: {log.contentId}</span>
                    )}
                  </td>

                  {/* 平台 */}
                  <td className="py-2 px-3 whitespace-nowrap">
                    <span className="text-sm">
                      {PLATFORM_ICONS[log.platform] ?? "📱"} {log.platform}
                    </span>
                  </td>

                  {/* 状态 */}
                  <td className="py-2 px-3 whitespace-nowrap">
                    <span className={`text-xs px-2 py-0.5 rounded ${STATUS_COLORS[log.status] ?? "bg-slate-700 text-slate-300"}`}>
                      {STATUS_ICONS[log.status] ?? ""} {log.status}
                    </span>
                  </td>

                  {/* 重试次数 */}
                  <td className="py-2 px-3 text-center text-xs text-slate-400">
                    {log.retryCount > 0 ? (
                      <span className="text-yellow-400">{log.retryCount}</span>
                    ) : (
                      <span>0</span>
                    )}
                  </td>

                  {/* 响应信息 */}
                  <td className="py-2 px-3 text-xs max-w-xs">
                    {log.response ? (
                      <span
                        className={`truncate block ${
                          log.status === "failed" ? "text-red-400" : "text-slate-400"
                        }`}
                        title={log.response}
                      >
                        {log.response}
                      </span>
                    ) : (
                      <span className="text-slate-600">-</span>
                    )}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
